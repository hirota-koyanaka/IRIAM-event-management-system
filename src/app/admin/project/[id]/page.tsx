  ...
  return (
    <details className="group bg-white/70 backdrop-blur-sm border border-white/50 rounded-xl shadow-md hover:shadow-lg transition-all duration-200">
      ...
      <div className="border-t border-gray-100 p-4 space-y-4">
        ...
        {/* 個別スケジュールURL */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">個別スケジュールURL</label>
          <div className="flex gap-2">
            <input
              type="text"
              value={`${typeof window !== 'undefined' ? window.location.origin : ''}/project/${project.id}/performer/${performer.id}`}
              readOnly
              className="flex-1 border-gray-200 rounded-xl px-4 py-2.5 border bg-gray-50 backdrop-blur-sm text-sm text-gray-600"
            />
            <button
              onClick={() => {
                const url = `${window.location.origin}/project/${project.id}/performer/${performer.id}`;
                ...
                if (btn) {
                  ...
                  setTimeout(() => {
                    btn.textContent = originalText || 'コピー';
            ...
            <a
              href={`/project/${project.id}/performer/${performer.id}`}
              target="_blank"
              rel="noopener noreferrer"
              className="px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-600 text-white rounded-xl hover:from-blue-600 hover:to-cyan-700 transition-all duration-200 text-sm font-medium flex items-center gap-1"
            >
              開く
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-xs text-gray-500 mt-1">このURLを出演者に共有すると、個別のスケジュールを確認できます</p>
        </div>
